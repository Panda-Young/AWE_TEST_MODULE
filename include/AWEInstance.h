...
/** OPTIONAL This callback is invoked from multiple places in the AWE framework to log any message to the BSP. User should register 
the callback via @ref awe_registerLoggingCallback, which requires logging level and type mask. type is a 32-bit bitfield, with each bit 
representing a type of message. These types are defined in the framework and provide information about where the logging messages come from.
*/
typedef void (*cbAweLogging_t)(struct _AWEInstance* pAWE, INT32 level, UINT32 type, void* payload, INT32 payloadSizeInBytes);

/*------------------AWE EVENTS ------------------*/

/** OPTIONAL These callback are invoked only from the Event Module. Use the @ref awe_registerEventCallbacks to register them
*/

/** The Event Trigger callback is called when an Event Module is triggered in the layout. Depending on the Event Module configuration,
this may happen inline in the module processing, or it may happen during deferred processing. In the latter case, there may be 
a delay between the actual event trigger and calling this Event Trigger callback. 
If this returns a negative number, then it is assumed that the Event Trigger was not successfully processed and the module's 
numFailedTriggers parameter will increment. If there is no Event Trigger callback registered, then the Event Module can never
generate an event. 
*/
...
/** The Event Register callback is called by an Event Module when it is constructed. The userHandle is free to use for whatever custom purpose is required. The user is responsible
for managing any memory allocated to the userHandle. The user must not define a Event Register callback if no preparation is necessary to handle an Event. 
If a negative value is returned, then the module will fail to initialize and the layout will fail to load.
*/
...
/** The Event Deregister callback is called by the Event Module when it is destroyed. When this is called, the user is responsible for freeing any memory 
allocated to the userHandle in the Event Register callback.
...
/**
 * @brief Register the logging callback with required logging level and logging type mask. The type mask is used to filter out undesired event types
 * @param [in] pAWE                 AWE instance pointer
 * @param [in] cbAweLogging         Function pointer of the logging callback.
 * @param [in] logLevel             Logging level. 0 = AWE_LOG_LEVEL_NONE (DISABLED), 1 = AWE_LOG_LEVEL_ERROR, 2 = AWE_LOG_LEVEL_WARNING, 3 = AWE_LOG_LEVEL_INFO, 4 = AWE_LOG_LEVEL_DEBUG
 * @param [in] logTypeMask          Logging type mask. Combination of any logging type like AWE_LOG_SYSTEM_MSG | AWE_LOG_CPU_OVERFLOW_MSG. Use AWE_LOG_ALL_TYPE_MASK to subscribe to all logging message types
                                    Please note that logging messages never end with a newline so the user callback has to apply that as desired.
...
/**
 * @brief Register the event callbacks used by the Event Module. cbAweEventTrigger must be non-null. The cbAweEventRegister and cbAweEventDeregister can be null. 
 * @param [in] pAWE                     AWE instance pointer
 * @param [in] cbAweEventTrigger        Function pointer of the event trigger callback
 * @param [in] cbAweEventRegister       Function pointer of the event register callback (optional)
 * @param [in] cbAweEventDeregister     Function pointer of the event deregister callback (optional)
